...
//! Based on the "power of two choices" paradigm from load balancing theory.
...
///
/// Randomly selects two workers and routes to the one with lower load.
/// This simple algorithm provides near-optimal load balancing with O(1) time
/// complexity per decision, based on the "power of two choices" paradigm.
#[derive(Debug)]
...
impl PowerOfTwoPolicy {
    /// Create a new power-of-two policy
    pub fn new() -> Self {
        Self {
...
mod tests {
    ...
    fn test_power_of_two_selection() {
        ...
        let worker3 = BasicWorker::new("http://w3:8000".to_string());
        ...
        assert!(
            selected_counts[2] > selected_counts[1],
            "Worker3 ({}) should be selected more than Worker2 ({})",
            selected_counts[2],
            selected_counts[1]
        );
        assert!(
            selected_counts[1] > selected_counts[0],
            "Worker2 ({}) should be selected more than Worker1 ({})",
            selected_counts[1],
            selected_counts[0]
        );
    }
    ...
    fn test_power_of_two_with_cached_loads() {
        ...
        assert!(
            w2_selected > 70,
            "Worker2 should win most comparisons, got {}",
            w2_selected
        );
    }
